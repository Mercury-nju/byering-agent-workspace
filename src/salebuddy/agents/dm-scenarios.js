...
const SCENARIOS = Object.freeze({
  main: {
    ...
    messages: [
      ...
      ["main", "Byering · 幕僚长", "明白。我把目标拆成三道检查：线索有效性、意向评分、触达承接。线索猎人和数据分析师先交叉核验，销售顾问只接收 A 级客户。"],
      ...
      ["main", "Byering · 幕僚长", "首轮已收口：新增有效潜客 214 位，其中 A 级 47 位；已触达 31 位，收到有效回复 12 位。当前卡点是 6 位客户主页信息不足，我已安排二次核验。", {
        ...
        content: "# 潜客拓展项目执行简报\n\n## 今日结论\n\n- 新增有效潜客：214 位\n- A 级高意向：47 位\n- 已发送私信：31 位\n- 有效回复：12 位\n- 待二次核验：6 位\n\n## 决策建议\n\n优先让销售顾问承接明确提到预算、车型或到店时间的 12 位客户；信息不足的客户暂不触达，避免浪费账号额度。\n\n## 风险\n\n晚间直播场次评论增速较快，建议 20:00 前扩容一次采集队列。"
      }]
    ]
  },
  "Browser Agent": {
  ...
  "Search Agent": {
    ...
    messages: [
      ...
      ["Search Agent", "数据分析师", "正在做去重和语义复核。我会把预算、车型、城市、时间窗口作为强特征，把泛讨论和竞品营销号作为负向特征，并保留评分依据。"],
      ["user", "我", "A 级名单宁可少一点，销售跟进量有限。"],
      ["Search Agent", "数据分析师", "评分完成：214 位有效候选中，A 级 47 位、B 级 86 位、C 级 81 位。抽检 40 条后，A 级判断一致率 92.5%；另外标记了 6 条证据不足数据，暂不进入触达。", {
  ...
  "App Agent": {
    ...
    messages: [
      ...
      ["App Agent", "销售顾问", "已按价格、配置、置换、到店四类意图匹配首触话术。前两轮只建立信任，客户主动追问后再邀请留资或预约。"],
      ["user", "我", "有回复的客户单独列出来，写清楚下一步由谁接。"],
      ["App Agent", "销售顾问", "已触达 31 位 A 级客户，12 位有效回复，其中 5 位确认本周可到店、4 位需要报价、3 位仍在车型对比。高价值会话已逐条写入跟进记录。", {
        ...
        ...PROJECT_LEADS,
        createdBy: "销售顾问",
        content: "客户,首触时间,当前状态,客户回复摘要,下一步,负责人\n上海阿杰,10:14,预约意向,周六下午可以到店,确认门店与试驾车型,销售顾问\n小鹿要换车,10:26,报价需求,想看两款车落地价差,发送对比报价,销售顾问\n老周在杭州,10:42,置换评估,旧车已开6年约8万公里,收集车型与车况,销售顾问\n橘子汽水,11:03,已读未回,,次日一次轻提醒,Byering"
      }]
    ]
  },
  "File Agent": {
    ...
    messages: [
      ...
      ["File Agent", "内容策划", "第三版已完成并通过敏感承诺检查。每套都控制在 55 字内，保留可替换的车型和城市变量，销售顾问可以直接调用。", {
        name: "高意向首触话术-v3.md",
        type: "doc",
