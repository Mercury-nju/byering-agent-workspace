...
import { hasCloudComputer } from "../office-role-skin.js";
...
export function mountCloudDesktop({ teamLive, gateway } = {}) {
  ...
  function openFor(type) {
    if (!hasCloudComputer(type)) return;
  ...
  function workstationHit(event) {
    ...
    for (const ws of g.scene.workstations || []) {
      ...
      if (!hasCloudComputer(ws.agentType)) continue;
  ...
  function onCanvasPointer(event) {
    // The current office identity layer owns native-character hit routing.
    if (officeCanvas()?.dataset?.sbOfficeIdentityOwned === "1") return;
