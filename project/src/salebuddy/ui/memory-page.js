...
.sb-memory-map-page .sb-page-body{overflow:hidden;background:var(--sb-app-page-bg,#f7f8fb)}
  ...
  const renderInspector = () => {
    ...
    selected.slice().reverse().forEach((entry) => {
      ...
      remove.addEventListener("click", async () => {
        ...
        try {
          if (!gateway) return;
          await gateway.action("agent.memory.delete", { agentType: "main", entryId: entry.id });
    ...
    if (!gateway) {
      body.appendChild(el("div", "sb-memory-empty", "知识库未连接，暂不能修改记忆。"));
    ...
    add.addEventListener("submit", async (event) => {
      ...
      try {
        if (!gateway) return;
        await gateway.action("agent.memory.append", { agentType: "main", entry: { kind: selectedKind, text, scope: "agent", source: "user" } });
        input.value = "";
        await loadEntries();
