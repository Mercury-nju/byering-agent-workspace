...
import { endAllWork, beginWork, pushActivity, reportWorkError, finishWork } from "../agents/work-live.js";
...
import { rememberAuthorizedManagedAccount } from "./realtime-work.js";
...
const CSS = `
.sb-chat{display:flex;flex-direction:column;height:100%;background:var(--sb-app-page-bg,#f7f8fb)}
...
.sb-result-plan{margin-top:13px;padding-top:12px;border-top:1px solid rgba(15,15,15,.08)}
.sb-result-plan-title{font-size:12px;font-weight:700;color:#1F2329}
.sb-result-plan-decision{margin-top:5px;font-size:11px;line-height:1.6;color:#59676B}
.sb-result-plan-list{display:grid;gap:7px;margin-top:9px}
.sb-result-plan-row{padding:8px 9px;border-radius:8px;background:#F7F9FB}
.sb-result-plan-step{font-size:11px;font-weight:650;color:#303943}
.sb-result-plan-acceptance{margin-top:3px;font-size:10.5px;line-height:1.5;color:#6D7A89}
.sb-result-plan-meta{margin-top:9px;font-size:10.5px;line-height:1.55;color:#59676B}
...
.sb-chat-input{flex:1;resize:none;border:1px solid rgba(15,15,15,0.1);border-radius:14px;padding:13px 17px;font-size:15px;line-height:1.55;font-family:inherit;color:#1F2329;outline:none;max-height:140px;background:var(--sb-app-page-bg,#f7f8fb)}
...
SCRIPTS.analyze = {
  ...
  subs: [
    ...
    { skill: "客户简报", role: "提炼联系前必须知道的上下文", executor: "客户画像研究员", assign: "我把主页、作品、评论和历史触达整理成每人的联系前简报。", lines: ["正在补齐近期作品、评论和历史触达上下文。", "每条高优先级线索都绑定了切入点和证据时间。"], completion: "客户简报已完成，可以继续准备触达。" }
  ],
...
function syncTask(engine, patch) {
  ...
  // Every online runtime result also feeds the Results Center. This keeps the
  // task runtime as the fact source and prevents Agent-specific UI paths from
  // becoming a separate result pipeline.
  const resultSnapshot = patch.resultSnapshot || snapshot?.resultSnapshot;
  if (engine.online && resultSnapshot && typeof resultSnapshot === "object" && !Array.isArray(resultSnapshot)) {
    const account = snapshot?.resolvedAccounts?.[0] || resultSnapshot.account || {};
    prospectStore.ingestRun({
      resultSnapshot,
      taskId: engine.taskId,
      agentId: engine.runtime?.agentRun?.agentId || resultSnapshot.agentId || "generalist",
      agentName: engine.runtime?.agentRun?.name || resultSnapshot.agentName || "项目执行 Agent",
      sourceContext: {
        source: resultSnapshot.source || "真实任务",
        accountId: resultSnapshot.accountId || account.secId || account.sec_id || account.id || "",
        accountName: account.nickname || account.uniqueId || account.unique_id || "",
        accountUrl: account.profileUrl || account.profile_url || "",
        taskRunId: engine.remoteRunId || resultSnapshot.taskRunId || ""
      }
    });
  }
}
...
function startEngine(engine) {
  ...
  if (engine.online && engine.commandClient && !engine.remoteTaskCreated) {
    ...
    engine.remoteTaskPromise = createRemoteTask({
      ...
      scenario: engine.scriptKey,
      chiefDecisionId: engine.chiefDecisionId
    }).then((identity) => {
      ...
      return startRemoteTask({
        ...
        if (started.state === "RUNNING") engine.accessStage = "ready";
        else if (started.state === "WAITING_ACCESS") engine.accessStage = "required";
        else engine.accessStage = "requirement";
        startEngine(engine);
        return identity;
      });
    }).catch((error) => {
  ...
  if (engine.online) {
    const adapter = createGatewayEventAdapter({
      ...
      onEvent: (event) => {
        ...
        if (event.t === "sub-error") {
          reportWorkError(event.agentType || event.agentName || "main", event.text || "子任务执行失败");
          endAllWork();
        }
        if (event.t === "sub-log") {
        ...
        if (event.t === "task-error") {
          reportWorkError(event.agentType || event.agentName || "main", event.text || "任务执行失败");
          endAllWork();
  ...
  function deliverDemoEvent(event) {
    ...
    if (event.t === "task-blocked") {
      reportWorkError(event.agentType || event.agentName || "main", event.text || "任务需要人工处理");
      endAllWork();
...
function ensureEngine({ taskId, taskText, projectId, projectName, projectMembers = [], teamLive, gateway = null, online = false, demoMode = false, runtimeEvents = [], taskStatus = null, taskResultSnapshot = null, taskPreview = "", remoteTaskId = null, remoteTaskRunId = null, remoteConversationId = null, remoteTaskVersion = null, remoteTaskSeq = null, browserSessionId = null, chiefDecisionId = null }) {
  ...
  const engine = {
    ...
    chiefDecisionId: chiefDecisionId || null,
...
function openConversation(engine) {
  ...
  function agentMsg(who, { typing = false, instant = false, avatarValue = who, messageClass = "", showActivity = true } = {}) {
    ...
    const activity = showActivity
      ? createAgentActivityBadge(avatarValue === "main" ? "main" : who, { status: { state: "working" } })
      : null;
  ...
  function showAccessRequired(setup, instant) {
    const localMode = engine.gateway?.cloudDesktopMode === "local" && typeof engine.gateway?.douyinStart !== "function";
  ...
  async function openAccessWindow(setup) {
    ...
    try {
      ...
      const douyinStart = engine.gateway?.douyinStart;
      const douyinOpenLogin = engine.gateway?.douyinOpenLogin;
      const douyinStatus = engine.gateway?.douyinStatus;
      if (provider === "douyin" && engine.online && typeof douyinStart === "function" && typeof douyinOpenLogin === "function" && typeof douyinStatus === "function") {
        const started = await douyinStart();
        if (started?.ok === false) throw new Error(started?.error?.message || "抖音云电脑启动失败");
        syncTask(engine, { status: "progress", preview: "云电脑已启动，正在准备抖音授权页面…" });
        const login = await douyinOpenLogin();
        if (login?.ok === false || !(login?.view_url || login?.login_url || login?.view_page_url)) {
          throw new Error(login?.error?.message || "云电脑没有返回登录页面");
        }
        ...
        const session = {
          sessionId: `douyin-mcp:${Date.now()}`,
          agentId: engine.runtimeDefinition?.agent?.id || engine.runtime?.agentRun?.agentId || null,
          pageUrl,
          cloudViewUrl: pageUrl,
          cloudDesktop: true,
          source: "douyin-mcp",
          accountKey: setup.account
        };
        engine.browserSessionId = session.sessionId;
        syncTask(engine, { status: "progress", preview: "云电脑已打开，等待你在云电脑内完成抖音授权…" });
        const checkAuthorization = async () => {
          const status = await douyinStatus();
          ...
          if (!loggedIn) return { state: "PENDING", raw: status };
          return { state: "READY", authenticationVerified: true, accountIdentity: identity, raw: status };
        };
        av.authWindow = openDouyinAuthorization({
          account: setup.account,
          scopes: setup.scopes,
          session,
          refreshCloudView: async () => douyinOpenLogin(),
          checkAuthorization,
          onAuthorized: ({ session: authorizedSession } = {}) => {
            const identity = authorizedSession?.accountIdentity || authorizedSession?.identity || null;
            if (authorizedSession?.state === "READY") {
              if (!identity || !(identity.uniqueId || identity.profileUrl || identity.uid || identity.secId)) {
                throw new Error("已检测到抖音登录，但暂未识别账号身份；不能创建托管账号");
              }
              rememberAuthorizedManagedAccount({
                id: session.accountKey,
                name: identity.accountName || setup.account,
                handle: identity.uniqueId ? `@${identity.uniqueId}` : "",
                status: "运行中",
                computer: "云电脑在线",
                identity,
              ...
              engine.gateway?.douyinStartNotificationMode?.().catch(() => {});
            }
            av.authWindow = null;
      ...
      // Douyin authorization must stay on the remote MCP cloud computer.
      // Never fall back to the legacy local browser workspace.
      if (provider === "douyin") {
        throw new Error("Douyin MCP 未连接，无法打开云电脑授权窗口");
      }
      ...
      engine.browserSessionId = session.sessionId;
      syncTask(engine, { status: "progress", preview: `${engine.gateway?.cloudDesktopMode === "local" && typeof engine.gateway?.douyinStart !== "function" ? "本机浏览器" : "云电脑"}已打开，等待抖音账号登录…` });
      av.authWindow = openDouyinAuthorization({
        account: setup.account,
        scopes: setup.scopes,
        session,
        ...
        onAuthorized: ({ session: authorizedSession } = {}) => {
          ...
          if (authorizedSession?.state === "READY") {
            ...
            rememberAuthorizedManagedAccount({
              id: session.accountKey,
              name: identity.accountName || authorizedSession.accountLabel || setup.account,
              handle: identity.uniqueId ? `@${identity.uniqueId}` : "",
              status: "运行中",
              computer: "在线",
              identity,
              authenticationVerified: authorizedSession?.authenticationVerified === true,
  ...
  function markAccessCancelled() {
    ...
    av.authButton.textContent = `${engine.gateway?.cloudDesktopMode === "local" && typeof engine.gateway?.douyinStart !== "function" ? "重新打开本机浏览器" : "重新打开云电脑"}`;
    av.authCard.querySelector(".sb-access-note")?.remove();
  }
  ...
  function showAssignmentPlan(assignments = [], text = "", instant = false, workflowId = null) {
    ...
    card.appendChild(el(
      "div",
      "sb-assignment-note",
      workflowId === "plan_only"
        ? "方案已经完成，不会读取业务数据，也不会调用外部执行器。"
        : requiresAccess
  ...
  function showSummaryBox(instant) {
    ...
    const bubble = agentMsg(chief, { instant, avatarValue: "main", showActivity: false });
    ...
    const planItems = Array.isArray(resultSnapshot.plan) ? resultSnapshot.plan : [];
    const acceptanceCriteria = Array.isArray(resultSnapshot.acceptanceCriteria) ? resultSnapshot.acceptanceCriteria.filter(Boolean) : [];
    const risks = Array.isArray(resultSnapshot.risks) ? resultSnapshot.risks.filter(Boolean) : [];
    const hasStructuredDetails = Boolean(resultSnapshot.decision || planItems.length || acceptanceCriteria.length || risks.length);
    if (hasStructuredDetails) {
      const plan = el("div", "sb-result-plan");
      plan.appendChild(el("div", "sb-result-plan-title", "执行方案"));
      if (resultSnapshot.decision) plan.appendChild(el("div", "sb-result-plan-decision", cleanEmployeeText(resultSnapshot.decision)));
      if (planItems.length) {
        const list = el("div", "sb-result-plan-list");
        planItems.forEach((item, index) => {
          const row = el("div", "sb-result-plan-row");
          row.appendChild(el("div", "sb-result-plan-step", `${item.step || index + 1}. ${item.agentName || "幕僚长"} · ${item.responsibility || item.mission || "任务规划"}`));
          if (item.acceptance) row.appendChild(el("div", "sb-result-plan-acceptance", `验收标准：${item.acceptance}`));
          list.appendChild(row);
        });
        plan.appendChild(list);
      }
      if (acceptanceCriteria.length) plan.appendChild(el("div", "sb-result-plan-meta", `验收标准：${acceptanceCriteria.join("；")}`));
      if (risks.length) plan.appendChild(el("div", "sb-result-plan-meta", `边界与风险：${risks.join("；")}`));
    ...
    if (engine.online && !metrics.length && !hasStructuredDetails) card.appendChild(el("div", "sb-access-note", "后端任务已完成，结构化业务结果将在数据返回后补充；当前没有展示示例指标。"));
  ...
  function renderEvent(event, instant) {
    switch (event.t) {
      ...
      case "assignment-plan":
        ...
        showAssignmentPlan(event.assignments || [], event.text, instant, event.workflowId);
        break;
      case "auth-required":
        if (engine.online) engine.accessStage = "required";
...
export async function startSkillTask({ name, prompt, teamLive = null, gateway = null, chiefDecisionId = null, openTaskConversation = true } = {}) {
  ...
  // A task can be launched from an existing conversation. In that case keep
  // the current conversation visible and let the caller report the outcome.
  if (openTaskConversation) openConversation(engine);
  else startEngine(engine);
  return true;
}
...
function explicitDemoMode() {
  ...
  return localHost || config.allowDemoMode === true;
}
