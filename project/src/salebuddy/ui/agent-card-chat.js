...
const cardAvatarOriginals = new WeakMap();
...
function readCardName(card) {
  ...
  return avatar?.getAttribute("alt")?.trim() || card.querySelector("[data-sb-card-avatar]")?.getAttribute("aria-label")?.trim() || null;
}

export { CARD_SELECTOR, readCardName, resolveAgentType };

function mountCardAvatar(card, agentType) {
  const image = card.querySelector("img[alt]");
  if (!image || image.dataset.sbCardAvatarSource) return;
  const original = image.cloneNode(false);
  const wrapper = document.createElement("span");
  wrapper.className = image.className;
  if (image.getAttribute("style")) wrapper.setAttribute("style", image.getAttribute("style"));
  wrapper.dataset.sbCardAvatar = "1";
  wrapper.dataset.sbCardAvatarSource = "1";
  wrapper.setAttribute("aria-label", image.getAttribute("alt") || `${agentType}头像`);
  cardAvatarOriginals.set(wrapper, original);
  image.replaceWith(wrapper);
  mountAgentAvatar(wrapper, agentType, { alt: wrapper.getAttribute("aria-label"), mode: "agent-card-chat" });
}

function restoreCardAvatar(wrapper) {
  const original = cardAvatarOriginals.get(wrapper);
  if (!original || !wrapper.isConnected) return;
  wrapper.replaceWith(original);
  cardAvatarOriginals.delete(wrapper);
}
...
export function mountAgentCardChat({ teamLive, onChat, onProgress, onCloud } = {}) {
  ...
  return {
    unmount() {
      observer.disconnect();
      document.querySelectorAll(`${CARD_SELECTOR} [data-sb-card-avatar]`).forEach(restoreCardAvatar);
    }
  };
}
