...
const DIRECT_MESSAGE_TIMEOUT_MS = 8000;
...
const COMMAND_ENVELOPE_FIELDS = new Set([
  ...
  "conversationId", "accountId", "agentId", "expectedVersion", "causationId", "correlationId",
  "actor", "createdAt", "metadata", "schemaVersion", "payload", "action"
]);

const CONTROL_PLANE_ACTIONS = new Set([
  "task.create", "task.run.start", "task.requirement.request", "task.requirement.edit",
  "task.config.update", "task.configuration.update", "task.strategy.update", "task.touch_content.update",
...
export class ControlPlaneHttpClient {
  constructor({
    ...
    directMessageTimeoutMs = DIRECT_MESSAGE_TIMEOUT_MS,
    apiKey = null,
    ...
    this.directMessageTimeoutMs = Math.max(100, Number(directMessageTimeoutMs) || DIRECT_MESSAGE_TIMEOUT_MS);
  ...
  async action(actionName, payload = {}, options = {}) {
    ...
    if (actionName === "chief.message.decide") {
      return this.request("/v1/chief/messages/decision", {
        method: "POST",
  ...
  async run(payload = {}, options = {}) {
    ...
    if (snapshot?.browserSessionId) {
      if (String(snapshot.browserSessionId).startsWith("douyin-mcp:")) await this.douyinStatus(options);
      else await this.browserSessionAuthorize(snapshot.browserSessionId, options);
    }
    let ack;
    if (["RUNNING", "WAITING_ACCESS", "WAITING_APPROVAL", "PAUSED", "WAITING_REPLY", "HANDOFF_REQUIRED"].includes(state)) {
      ...
      ack = await this.action("task.run.start", {
        ...
        payload: {
          ...
          // any access grant) has produced a RUNNING snapshot. The server may
          // auto-confirm only when its persisted decision marks the task as
          // low-risk and complete; this flag is never trusted on its own.
          requirementsConfirmed: payload.requirementsConfirmed === true,
          autoConfirmLowRisk: payload.autoConfirmLowRisk !== false,
  ...
  async douyinStart({ billingPlan = "monthly" } = {}, options = {}) {
    return this.request("/v1/douyin/mcp/start", {
      method: "POST",
      body: JSON.stringify({ billingPlan })
    }, { timeoutMs: 375000, ...options });
  }

  async douyinRestart({ agentId, billingPlan = "monthly" } = {}, options = {}) {
    return this.request("/v1/douyin/mcp/restart", {
      method: "POST",
      body: JSON.stringify({ agentId, billingPlan })
    }, { timeoutMs: 375000, ...options });
  }

  async douyinStatus(options = {}) {
    return this.request("/v1/douyin/mcp/status", { method: "GET" }, options);
  }

  async douyinOpenLogin(options = {}) {
    return this.request("/v1/douyin/mcp/open-login", { method: "POST", body: "{}" }, options);
  }

  async douyinStartNotificationMode(options = {}) {
    return this.request("/v1/douyin/mcp/start-notification-mode", { method: "POST", body: "{}" }, options);
  }

  async douyinPullNotifications({ cursor = 0, limit = 20, waitMs = 0, agentId = null } = {}, options = {}) {
    return this.request("/v1/douyin/mcp/pull-notifications", {
      method: "POST",
      body: JSON.stringify({ cursor, limit, waitMs, ...(agentId ? { agentId } : {}) })
    }, { timeoutMs: Math.max(this.timeoutMs, Math.min(30000, Number(waitMs) || 0) + 10000), ...options });
  }

  async snapshot(taskId) {
    if (!taskId) throw new ControlPlaneHttpError("任务快照需要 taskId", { code: "TASK_ID_REQUIRED" });
    const snapshot = await this.request(`/v1/tasks/${encodeURIComponent(taskId)}`, { method: "GET" });
    return normalizeControlPlaneTaskSnapshot(snapshot);
  }
...
/** Normalize acquisition status at the durable transport boundary as well as work-live. */
export function normalizeControlPlaneTaskSnapshot(snapshot) {
  if (!isRecord(snapshot)) return snapshot;
  const state = snapshot.acquisitionTaskState ?? snapshot.acquisition_task_state;
  const normalized = normalizeAcquisitionTaskStatus(state);
...
export function createHybridGateway({ nativeGateway = null, controlPlane = null } = {}) {
  ...
  const hybrid = {
    ...
    douyinStart(payload = {}, options = {}) {
      if (!controlPlane?.douyinStart) return Promise.reject(new Error("Douyin MCP 未连接"));
      return controlPlane.douyinStart(payload, options);
    },
    douyinStatus(options = {}) {
      if (!controlPlane?.douyinStatus) return Promise.reject(new Error("Douyin MCP 未连接"));
      return controlPlane.douyinStatus(options);
    },
    douyinOpenLogin(options = {}) {
      if (!controlPlane?.douyinOpenLogin) return Promise.reject(new Error("Douyin MCP 未连接"));
      return controlPlane.douyinOpenLogin(options);
    },
    douyinStartNotificationMode(options = {}) {
      if (!controlPlane?.douyinStartNotificationMode) return Promise.reject(new Error("Douyin MCP 未连接"));
      return controlPlane.douyinStartNotificationMode(options);
    },
    subscribeTask(taskId) {
...
export function toAgUiEvent(event) {
  ...
  const normalizedPayload = normalizeControlPlaneTaskSnapshot(payload);
  const base = {
  ...
  switch (event.type) {
    ...
    case "task.config.updated":
    case "task.configuration.updated":
    case "task.strategy.updated":
    case "task.touch_content.updated":
      return { ...base, type: "TASK_CONFIG_UPDATED", ...payload };
    case "task.assignment.proposed": return {
      ...base,
      ...payload,
      type: "ASSIGNMENT_PROPOSED",
      assignments: payload.assignments || [],
      workflowId: payload.workflow?.id || payload.workflowId || null
    };
    ...
    case "outreach.accepted": return { ...executionPayload, type: "OUTREACH_ACCEPTED", deliveryState: "accepted" };
    case "outreach.sent": return { ...executionPayload, type: "OUTREACH_SENT" };
    case "reply.sent": return { ...executionPayload, type: "OUTREACH_SENT", deliveryState: payload.deliveryState || "sent" };
