...
export const AGENT_TYPE_DEFAULTS = Object.freeze({
  ...
  "App Agent": {
    role: "触达策略师",
    title: "触达策略师",
    responsibilities: ["制定首轮触达方式和批次节奏", "基于证据生成个性化首触", "评估触达结果与下一步动作"]
  },
  "Risk Agent": {
    role: "风控专员",
    title: "触达风险控制",
  ...
  "Outreach Ops Agent": {
    role: "触达运营专员",
    title: "触达运营专员",
    responsibilities: ["管理发送队列和分批计划", "处理失败、重试和暂停恢复", "监听回复事件并停止后续自动计划"]
  },
  "File Agent": {
    role: "内容策划",
    title: "内容与文档产出",
...
export function reviseMemoryEntry(entry, text) {
  ...
  return { ...entry, text: String(text || ""), version: entry.version + 1, history, status: "active", archivedAt: null, archiveReason: null, updatedAt: new Date().toISOString() };
}
...
export function rollbackMemoryEntry(entry) {
  ...
  return { ...entry, text: previous.text, version: previous.version, history, status: "active", archivedAt: null, archiveReason: null, updatedAt: new Date().toISOString() };
}
