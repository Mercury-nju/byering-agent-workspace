...
export const COMMAND_TYPES = Object.freeze({
  ...
  TASK_CONFIG_UPDATE: "task.config.update",
...
export function normalizeCommand(input = {}) {
  ...
  if (type === COMMAND_TYPES.TASK_CONFIG_UPDATE) normalizeTaskConfigurationPayload(payload);
  ...
  if (type === COMMAND_TYPES.TASK_CONFIG_UPDATE && expectedVersion == null) {
    throw new TaskProtocolError("expectedVersion is required for configuration updates", "CONFIGURATION_TASK_VERSION_REQUIRED", { field: "expectedVersion" });
  }
...
function resolveTransition(state, type, payload) {
  ...
  switch (type) {
    ...
    case COMMAND_TYPES.REQUIREMENT_REQUEST:
      return [TASK_STATES.CREATED, TASK_STATES.WAITING_REQUIREMENT, TASK_STATES.RUNNING].includes(state)
    ...
    case COMMAND_TYPES.TASK_CONFIG_UPDATE:
      return [TASK_STATES.RUNNING, TASK_STATES.PAUSED].includes(state) ? state : null;
    ...
    case COMMAND_TYPES.BLOCK:
      return [TASK_STATES.RUNNING, TASK_STATES.WAITING_ACCESS, TASK_STATES.WAITING_REPLY].includes(state) ? TASK_STATES.BLOCKED : null;
    default:
      return null;
  }
}
...
function validateCommandPayload(type, payload) {
  switch (type) {
    ...
    case COMMAND_TYPES.REPLY:
      ...
      return;
    case COMMAND_TYPES.TASK_CONFIG_UPDATE:
      validateTaskConfigurationUpdatePayload(payload);
      return;
    default:
      return;
  }
...
const CONFIGURATION_CHANGE_KEYS = new Set([
  "findingStrategy",
  "touchChannel",
  "touchContent",
  "replyStyle",
...
const CONFIGURATION_PAYLOAD_KEYS = new Set([
  "baseConfigVersion",
  "configVersion",
  "effectiveScope",
  "changes",
...
function validateTaskConfigurationUpdatePayload(payload) {
  if (!Number.isInteger(payload.baseConfigVersion) || payload.baseConfigVersion < 1) {
    throw new TaskProtocolError("baseConfigVersion must be a positive integer", "CONFIGURATION_VERSION_REQUIRED", { field: "baseConfigVersion" });
  }
  if (!Number.isInteger(payload.configVersion) || payload.configVersion < 1) {
    throw new TaskProtocolError("configVersion must be a positive integer", "CONFIGURATION_VERSION_REQUIRED", { field: "configVersion" });
  }
  if (payload.configVersion !== payload.baseConfigVersion + 1) {
    throw new TaskProtocolError("configVersion must increment baseConfigVersion by one", "INVALID_CONFIGURATION_VERSION", {
      baseConfigVersion: payload.baseConfigVersion,
      configVersion: payload.configVersion
    });
  }
  if (payload.effectiveScope !== "future_only") {
    throw new TaskProtocolError("Configuration changes only apply to future execution", "INVALID_EFFECTIVE_SCOPE", { field: "effectiveScope" });
  }
  if (!isRecord(payload.changes) || Object.keys(payload.changes).length === 0) {
    throw new TaskProtocolError("At least one configuration change is required", "CONFIGURATION_CHANGES_REQUIRED", { field: "changes" });
  }
  for (const key of Object.keys(payload)) {
    if (!CONFIGURATION_PAYLOAD_KEYS.has(key)) {
      throw new TaskProtocolError(`Configuration update field is not supported: ${key}`, "CONFIG_UPDATE_FIELD_UNSUPPORTED", { field: key });
    }
  }
  for (const key of Object.keys(payload.changes)) {
    if (!CONFIGURATION_CHANGE_KEYS.has(key)) {
      throw new TaskProtocolError(`Configuration update field cannot be changed: ${key}`, "CONFIG_UPDATE_FIELD_FORBIDDEN", { field: key });
    }
    rejectForbiddenConfigurationKeys(payload.changes[key], `changes.${key}`);
  }
  if (payload.confirmation !== undefined && !isRecord(payload.confirmation)) {
    throw new TaskProtocolError("confirmation must be an object", "INVALID_CONFIGURATION_CONFIRMATION", { field: "confirmation" });
  }
  if (isRiskyConfigurationUpdate(payload.changes) && payload.confirmation?.confirmed !== true) {
    throw new TaskProtocolError("This configuration change requires explicit confirmation", "CONFIG_UPDATE_CONFIRMATION_REQUIRED");
  }
}

function normalizeTaskConfigurationPayload(payload) {
  if (!isRecord(payload.changes)) return;
  const changes = payload.changes;
  const normalized = { ...changes };

  if (isRecord(changes.strategy) || isRecord(changes.discovery)) {
    normalized.findingStrategy = {
  ...
  if (isRecord(changes.runtimeRules) || isRecord(changes.runtime)) {
    ...
    if (runtime.frequency !== undefined || runtime.maxTouchesPerDay !== undefined || runtime.minIntervalMinutes !== undefined) {
      const currentFrequency = runtime.frequency;
      normalized.frequency = isRecord(currentFrequency)
        ? { ...currentFrequency, ...Object.fromEntries(["maxTouchesPerDay", "minIntervalMinutes"].filter((key) => runtime[key] !== undefined).map((key) => [key, runtime[key]])) }
        : {
            ...(currentFrequency !== undefined ? { mode: currentFrequency } : {}),
    ...
    if (runtime.stopConditions !== undefined) normalized.stopConditions = runtime.stopConditions;
    delete normalized.runtimeRules;
    delete normalized.runtime;
  }

  if (payload.confirmation === true) payload.confirmation = { confirmed: true };
  payload.changes = normalized;
}

function rejectForbiddenConfigurationKeys(value, path) {
  if (Array.isArray(value)) {
    value.forEach((item, index) => rejectForbiddenConfigurationKeys(item, `${path}[${index}]`));
    return;
  }
  if (!isRecord(value)) return;
  for (const [key, child] of Object.entries(value)) {
    const policyKey = String(key).replace(/[_-]/g, "").toLowerCase();
    const forbidden = [...FORBIDDEN_CONFIGURATION_KEYS].some((candidate) => (
      String(candidate).replace(/[_-]/g, "").toLowerCase() === policyKey
    ));
    if (forbidden) {
      throw new TaskProtocolError(`Configuration update cannot change ${path}.${key}`, "CONFIG_UPDATE_FIELD_FORBIDDEN", { field: `${path}.${key}` });
    }
    rejectForbiddenConfigurationKeys(child, `${path}.${key}`);
  }
}

function isRiskyConfigurationUpdate(changes) {
  if (changes.approvalMode === "auto" || changes.touchContent?.approvalMode === "auto") return true;
  const strategy = changes.findingStrategy;
  if (isRecord(strategy) && (
    strategy.scopeExpansion === true
    || strategy.expandScope === true
  )) return true;
  const frequency = changes.frequency;
  if (isRecord(frequency)) {
    if (frequency.relaxed === true || frequency.allowIncrease === true || frequency.increaseBy > 0 || frequency.maxPerDayIncrease > 0 || frequency.maxTouchesPerDayIncrease > 0) return true;
    if (frequency.maxTouchesPerDay != null && frequency.previousMaxTouchesPerDay != null && frequency.maxTouchesPerDay > frequency.previousMaxTouchesPerDay) return true;
  }
  return false;
}
