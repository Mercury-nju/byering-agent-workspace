...
const EVENT_TYPES = Object.freeze({
  ...
  "outreach-accepted": "outreach.accepted",
  ...
  "task-config-updated": "task.config.updated"
});
...
const INITIAL_SNAPSHOT = Object.freeze({
  ...
  resultSnapshot: null,
  artifacts: [],
  configuration: { version: 1, effectiveScope: "task_start" }
});
...
function applyEvent(snapshot, event) {
  ...
  if (type === "task.config.updated") {
    const supplied = event.configuration || event.config || null;
    if (supplied && typeof supplied === "object" && !Array.isArray(supplied)) {
      next.configuration = {
        ...(next.configuration || {}),
        ...clone(supplied)
      };
    } else if (Number.isInteger(event.configurationVersion)) {
      next.configuration = {
        ...(next.configuration || {}),
        version: event.configurationVersion,
        effectiveScope: event.effectiveScope || "future_only"
      };
    }
  }
  ...
  if (resultEvent) {
    ...
    if (type === "outreach.accepted" || type === "outreach.sent" || type === "reply.sent" || type === "outreach.failed") {
      ...
      const key = type === "outreach.accepted" ? "accepted" : type === "outreach.failed" ? "failed" : "sent";
