...
import { routeChiefDouyinTask } from "../agents/chief-douyin-agent-routing.js";
...
// This is a runtime routing decision, separate from the user-facing workflow
// name. A find-only task can be either public discovery or an account-bound
// product Agent run, so workflow id alone must never select an executor.
export const EXECUTION_BOUNDARIES = Object.freeze({
  PLANNING: "planning",
  LEGACY_PUBLIC_DISCOVERY: "legacy_public_discovery",
  PRODUCT_AGENTS: "product_agents"
});

const PLAN_AGENT_IDS = Object.freeze(["chief_of_staff"]);
...
const WORKFLOW_DEFINITIONS = deepFreeze({
  [WORKFLOW_IDS.PLAN_ONLY]: {
    id: WORKFLOW_IDS.PLAN_ONLY,
    displayName: "方案规划工作流",
    description: "把已确认目标整理为任务拆解、Agent 分工、验收标准和风险边界，不读取业务数据，不执行外部动作。",
    agentIds: PLAN_AGENT_IDS,
    requiresAccess: false,
    allowsOutreach: false,
    completionAgent: "chief_of_staff",
    approvalBefore: null,
    tools: ["strategy_brief", "task_planner", "evidence_review"]
  },
...
const CAPABILITY_WORKFLOWS = Object.freeze({
  douyin_account_discovery: WORKFLOW_IDS.FIND_ONLY,
  douyin_comment_analysis: WORKFLOW_IDS.FIND_ONLY,
  douyin_private_outreach: WORKFLOW_IDS.FIND_AND_OUTREACH,
  douyin_public_reply: WORKFLOW_IDS.FIND_AND_OUTREACH,
  douyin_inbox_reply: WORKFLOW_IDS.FIND_AND_OUTREACH
});

const CAPABILITY_EXECUTORS = Object.freeze({
  douyin_account_discovery: ["lead_miner"],
  douyin_comment_analysis: ["lead_miner", "lead_analyst"],
  douyin_private_outreach: ["outreach_operator"],
  douyin_public_reply: ["outreach_operator"],
  douyin_inbox_reply: ["outreach_operator"]
});

const ASSIGNMENT_GRAPH = Object.freeze({
  chief_of_staff: { dependsOn: [], parallelGroup: "coordination" },
  acquisition_strategist: { dependsOn: ["chief_of_staff"], parallelGroup: "planning" },
  lead_miner: { dependsOn: ["acquisition_strategist"], parallelGroup: "discovery" },
  lead_analyst: { dependsOn: ["lead_miner"], parallelGroup: "qualification" },
  prospect_researcher: { dependsOn: ["lead_miner"], parallelGroup: "qualification" },
  sales_consultant: { dependsOn: ["lead_analyst", "prospect_researcher"], parallelGroup: "strategy" },
  risk_specialist: { dependsOn: ["lead_analyst", "prospect_researcher"], parallelGroup: "risk" },
  outreach_specialist: { dependsOn: ["sales_consultant", "risk_specialist"], parallelGroup: "outreach_preparation" },
  outreach_operator: { dependsOn: ["outreach_specialist", "risk_specialist"], parallelGroup: "external_execution" }
});
...
export function selectWorkflowForRequirement(requirement = {}, options = {}) {
  if (isPlanOnlyRequirement(requirement, options)) {
    return getWorkflowDefinition(WORKFLOW_IDS.PLAN_ONLY);
  }
...
export function resolveExecutionBoundary(requirement = {}, {
  taskAgentId = "",
  requiresAccess = false,
  goal = ""
} = {}) {
  if (isPlanOnlyRequirement(requirement, { requiresAccess, goal })) {
    return EXECUTION_BOUNDARIES.PLANNING;
  }
  if (requestedProductAgentId(requirement, taskAgentId)) {
    return EXECUTION_BOUNDARIES.PRODUCT_AGENTS;
  }
  return classifyRequirement(requirement, { requiresAccess, goal }).requiresExternalAccess
...
export function assignmentPlanForWorkflow(workflowId) {
  ...
  return workflow.agentIds.map((agentId, index) => {
    ...
    const contract = workflow.id === WORKFLOW_IDS.PLAN_ONLY && agentId === "chief_of_staff"
      ? {
          ...WORKFLOW_AGENT_CONTRACTS[agentId],
          skillId: "execution_planning",
          skill: "执行方案规划",
          acceptance: "任务拆解、责任 Agent、验收标准和风险边界已形成"
        }
      : WORKFLOW_AGENT_CONTRACTS[agentId];
    const graph = ASSIGNMENT_GRAPH[agentId] || { dependsOn: [], parallelGroup: `stage_${index}` };
    return {
      index,
      agentId,
      ...
      executionRole: agentId === "chief_of_staff" ? "coordination" : "specialist",
      dependsOn: [...graph.dependsOn],
      parallelGroup: graph.parallelGroup
    };
  });
}

export function buildCapabilityAssignmentPlan(requirement = {}, {
  availableAgentIds,
  requiresAccess = false,
  goal = "",
  productRouting = true
} = {}) {
  const capabilities = Array.isArray(requirement?.decision?.requiredCapabilities)
    ? [...new Set(requirement.decision.requiredCapabilities.filter((item) => typeof item === "string" && item.trim()))]
    : [];
  const baselineWorkflow = selectWorkflowForRequirement(requirement, { requiresAccess, goal });
  const selectedWorkflowId = baselineWorkflow.id === WORKFLOW_IDS.PLAN_ONLY
    ? WORKFLOW_IDS.PLAN_ONLY
    : capabilities.map((capability) => CAPABILITY_WORKFLOWS[capability]).find((id) => id === WORKFLOW_IDS.FIND_AND_OUTREACH)
      || capabilities.map((capability) => CAPABILITY_WORKFLOWS[capability]).find(Boolean)
      || baselineWorkflow.id;
  const workflow = getWorkflowDefinition(selectedWorkflowId);
  const productRoute = productRouting
    ? routeChiefDouyinTask({
        requiredCapabilities: capabilities,
        requestedAgentId: requirement?.decision?.requestedAgentId || requirement?.requestedAgentId || requirement?.agentId || "",
        availableAgentIds
      })
    : null;
  if (productRoute?.applicable) {
    return {
      workflow,
      assignments: productRoute.assignments,
      requiredCapabilities: capabilities,
      missingCapabilities: productRoute.blocked ? capabilities : [],
      missingAgentIds: productRoute.missingAgentIds,
      incompatibleCapabilities: productRoute.incompatibleCapabilities || [],
      blocked: productRoute.blocked,
      productRoute
    };
  }
  const available = availableAgentIds == null ? new Set(workflow.agentIds) : new Set(availableAgentIds);
  const missingCapabilities = capabilities.filter((capability) => {
    const executors = CAPABILITY_EXECUTORS[capability];
    return !executors?.some((agentId) => available.has(agentId));
  });
  const assignments = assignmentPlanForWorkflow(workflow.id).filter((assignment) => available.has(assignment.agentId));
  return {
    workflow,
    assignments,
    requiredCapabilities: capabilities,
    ...
    blocked: missingCapabilities.length > 0,
    productRoute: null
  };
}

function requestedProductAgentId(requirement = {}, taskAgentId = "") {
  const candidates = [
    taskAgentId,
    requirement?.decision?.requestedAgentId,
    requirement?.requestedAgentId,
    requirement?.agentId
  ];
  return candidates
    .map((candidate) => typeof candidate === "string" ? candidate.trim() : "")
    .find((candidate) => DOUYIN_ACQUISITION_ACTIVE_AGENT_IDS.includes(candidate)) || "";
}
...
const PLAN_INTENT_PATTERN = /(?:计划|规划|方案|任务拆解|agent\s*分工|智能体分工|验收标准|执行节奏)/i;
const PLAN_ONLY_ACTION_PATTERN = /(?:只|仅)(?:需|需要|做|输出|生成|制定|包含|提供)?[^。；\n]{0,24}(?:计划|规划|方案|任务拆解|agent\s*分工|智能体分工|验收标准)/i;
const NO_DATA_EXECUTION_PATTERN = /不(?:读取|访问|采集|抓取|搜索|检索|核验|执行|发送|触达)[^。；\n]{0,16}(?:数据|账号|候选|用户|消息|私信|触达|采集|搜索|检索|外部动作)?/i;

function isPlanOnlyRequirement(requirement = {}, options = {}) {
  const action = requirementActionText(requirement);
  const text = [requirementText(requirement), textValue(options.goal)].filter(Boolean).join(" ");
  if (!PLAN_INTENT_PATTERN.test(text)) return false;
  const explicitPlanAction = PLAN_ONLY_ACTION_PATTERN.test(text)
    || /只做(?:规划|计划|方案)/i.test(action)
    || /不执行(?:数据)?(?:采集|抓取|搜索|检索)/i.test(action);
  const noExecutionBoundary = NO_DATA_EXECUTION_PATTERN.test(text);
  return explicitPlanAction && noExecutionBoundary;
}
