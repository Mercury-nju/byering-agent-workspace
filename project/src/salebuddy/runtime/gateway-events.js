...
function executionFields(source = {}) {
  ...
  for (const key of [
    ...
    "deliveryState", "content", "errorCode", "error_code", "reason", "retryable", "score", "tier", "count",
...
const REMOTE_AGENT_PRESENTATION = Object.freeze({
  ...
  prospect_researcher: Object.freeze({ agentName: "客户画像研究员", skill: "线索证据简报" }),
...
export function createGatewayEventAdapter({ taskId = null, onEvent = null } = {}) {
  ...
  function accept(source = {}) {
    ...
    if (type === "RUN_STARTED") {
      push({ t: "run-started" });
      const state = String(source.state || source.data?.state || "").trim().toUpperCase();
      if (!["CREATED", "WAITING_REQUIREMENT", "WAITING_ACCESS", "WAITING_APPROVAL"].includes(state)) {
    ...
    } else if (["OUTREACH_ACCEPTED", "outreach.accepted"].includes(type)) {
      push({ t: "outreach-accepted", deliveryState: "accepted", ...executionFields(source) });
    } else if (["OUTREACH_SENT", "outreach.sent", "reply.sent"].includes(type)) {
      ...
      push({
        ...
        workflowId: source.workflowId || source.workflow_id || source.workflow?.id || source.data?.workflow?.id || null,
