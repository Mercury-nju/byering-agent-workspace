...
test("command client maps live configuration updates to the Gateway action", async () => {
  const calls = [];
  const client = createTaskCommandClient({
    gateway: {
      action: async (action, payload) => {
        calls.push({ action, payload });
        return { accepted: true, currentVersion: 8 };
      }
    }
  });

  const result = await client.send(COMMAND_TYPES.TASK_CONFIG_UPDATE, {
    taskId: "task-1",
    expectedVersion: 7,
    payload: {
      baseConfigVersion: 1,
      ...
      changes: { touchContent: "你好" }
    }
  });

  assert.equal(calls[0].action, "task.config.update");
  assert.equal(calls[0].payload.configVersion, 2);
  assert.equal(calls[0].payload.effectiveScope, "future_only");
  assert.equal(calls[0].payload.taskId, "task-1");
  assert.equal(calls[0].payload.expectedVersion, 7);
  assert.equal(result.accepted, true);
  assert.equal(client.actionFor(COMMAND_TYPES.TASK_CONFIG_UPDATE), "task.config.update");
});
