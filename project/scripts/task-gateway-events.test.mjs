...
test("a gated run start does not pretend execution has begun", () => {
  const events = [];
  const adapter = createGatewayEventAdapter({ taskId: "task-gated-1", onEvent: (event) => events.push(event) });
  ...
  assert.deepEqual(events.map(({ t }) => t), ["run-started"]);
});

test("a real execution acceptance opens progress before dispatch feedback", () => {
  const events = [];
  const adapter = createGatewayEventAdapter({ taskId: "task-execution-1", onEvent: (event) => events.push(event) });

  adapter.accept({
    type: "TASK_EXECUTION_ACCEPTED",
    event_id: "execution-accepted-1",
    status: "accepted",
    text: "任务已进入真实执行队列"
  });

  assert.deepEqual(events.map(({ t }) => t), ["progress-start", "dispatch"]);
});
...
test("plan-only assignment events preserve the workflow identity for the UI", () => {
  const events = [];
  const adapter = createGatewayEventAdapter({ taskId: "task-plan", onEvent: (event) => events.push(event) });
  adapter.accept({
    type: "ASSIGNMENT_PROPOSED",
    event_id: "evt-plan-assignment",
    workflow: { id: "plan_only" },
    assignments: [{ agentId: "chief_of_staff", skill: "执行方案规划" }],
    text: "幕僚长已形成执行方案"
  });

  assert.equal(events[0].t, "assignment-plan");
  assert.equal(events[0].workflowId, "plan_only");
});
...
test("ClueHunter execution facts become the same progress, outreach, lead, and artifact events as native AG-UI", () => {
  ...
  assert.equal(events[1].commandId, "cmd-1");
  assert.equal(events[2].source, "cluehunter");
  assert.equal(events[2].resultSnapshot.counts.leads, 1);
  assert.equal(events[3].leadId, "lead-1");
  assert.equal(events[4].replyText, "想了解方案");
  assert.equal(events[5].name, "线索.csv");
  assert.equal(events[6].resultSnapshot.counts.outreach, 1);
});
