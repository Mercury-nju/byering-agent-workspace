...
test("employee selection is the shared source for prompt cards", () => {
  ...
  assert.deepEqual(choices.map(({ agent }) => agent.name), ["账号发现与解析师", "线索猎人", "线索分析师", "客户画像研究员"]);
  assert.equal(hunter.length, 4);
  assert.ok(hunter.every(({ employeeName, prompt }) => employeeName === "线索猎人" && /^帮我/.test(prompt)));
  assert.ok(content.every(({ employeeName, prompt }) => employeeName === "客户画像研究员" && /^帮我/.test(prompt)));
...
test("sales shortcuts switch the visible team and task prompts together", () => {
  ...
  assert.deepEqual(shortcutAgentChoices("find").map(({ agent }) => agent.name), ["账号发现与解析师", "线索猎人", "线索分析师", "客户画像研究员"]);
  assert.deepEqual(shortcutAgentChoices("outreach").map(({ agent }) => agent.name), ["客户画像研究员", "风控专员", "线索分析师", "幕僚长"]);
...
test("homepage brand lockup pairs the logo mark with the product name", () => {
  const source = readFileSync(path.join(projectRoot, "src/salebuddy/ui/home-sales-feed.js"), "utf8");
  assert.match(source, /\.sb-home-hero-lockup\{[^}]*display:inline-flex;[^}]*height:52px;[^}]*overflow:visible/);
  assert.match(source, /\.sb-home-hero-mark\{[^}]*width:52px;height:52px;[^}]*object-fit:cover/);
  assert.match(source, /lockup\.append\(mark, name\)/);
});
...
test("homepage brand lockup stays visually subordinate to the hero navigation at desktop widths", () => {
  const source = readFileSync(path.join(projectRoot, "src/salebuddy/ui/home-sales-feed.js"), "utf8");
  assert.match(source, /@media\(max-width:1500px\)[\s\S]*?\.sb-home-hero-lockup\{height:46px\}/);
  assert.match(source, /@media\(max-width:1324px\)[\s\S]*?\.sb-home-hero-lockup\{height:42px\}/);
  assert.match(source, /@media\(max-width:760px\)[\s\S]*?\.sb-home-hero-lockup\{height:36px;gap:8px\}/);
});
