...
import { compactMemoryEntries } from "../src/salebuddy/agents/memory-lifecycle.js";
...
export function createAgentStore(root, { seedMessages = true } = {}) {
  return {
    ...
    getCompanion(agentType) {
      return readJson(path.join(ensureLayout(root, agentType), "companion.json"), null);
    },

    saveCompanion(agentType, state) {
      writeJsonAtomic(path.join(ensureLayout(root, agentType), "companion.json"), state);
      return state;
    },

    updateDm(agentType, messageId, patch) {
      const messages = this.listDm(agentType);
      const index = messages.findIndex(message => message.id === messageId);
      if (index < 0) return null;
      messages[index] = { ...messages[index], ...patch, id: messageId };
      writeJsonAtomic(path.join(ensureLayout(root, agentType), "dm.json"), messages);
    ...
    appendMemory(agentType, { kind, text, scope, source }) {
      ...
      const next = compactMemoryEntries([entry, ...entries]).entries;
    ...
    reviseMemory(agentType, entryId, text) {
      ...
      const next = compactMemoryEntries(entries).entries;
      writeJsonAtomic(file, next);
    ...
    listDm(agentType) {
      ...
      if (!seedMessages && messages.some((message) => String(message.id || "").startsWith("dm-seed-"))) {
        const durableMessages = messages.filter((message) => !String(message.id || "").startsWith("dm-seed-"));
        writeJsonAtomic(file, durableMessages);
    ...
    appendDm(agentType, { from, fromName, text, artifact = null, conversationId = null, metadata = null }) {
      const dir = ensureLayout(root, agentType);
      const file = path.join(dir, "dm.json");
      const messages = this.listDm(agentType);
      const message = {
    ...
    clearDm(agentType, { conversationId = null } = {}) {
      const dir = ensureLayout(root, agentType);
      const file = path.join(dir, "dm.json");
      const messages = this.listDm(agentType);
      const next = conversationId
        ? messages.filter((message) => message.conversationId !== String(conversationId))
        : [];
      writeJsonAtomic(file, next);
      return { deleted: messages.length - next.length, remaining: next.length };
    },

    removeDm(agentType, predicate) {
      if (typeof predicate !== "function") throw new TypeError("A message predicate is required");
      const dir = ensureLayout(root, agentType);
      const file = path.join(dir, "dm.json");
      const messages = this.listDm(agentType);
      const next = messages.filter((message) => !predicate({ ...message }));
      writeJsonAtomic(file, next);
      return { deleted: messages.length - next.length, remaining: next.length };
    },
