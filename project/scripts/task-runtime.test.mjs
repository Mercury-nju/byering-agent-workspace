...
test("runtime replay projects a live configuration update without changing task history", () => {
  const runtime = createRuntimeTask({ taskId: "task-runtime-config", taskText: "运行中的获客任务" });
  ...
  assert.equal(runtime.snapshot.configuration.version, 2);
  assert.equal(runtime.snapshot.configuration.touchContent, "测试");
  assert.equal(runtime.snapshot.taskState, "CREATED");
  assert.equal(runtime.events.length, 1);
  assert.equal(runtime.events[0].type, "task.config.updated");
});
