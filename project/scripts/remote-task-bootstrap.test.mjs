...
test("remote task bootstrap keeps server task, run, and conversation identities", async () => {
  ...
  const commandClient = {
    send: async (type, input) => {
      ...
      if (type === COMMAND_TYPES.TASK_CREATE) {
        return { accepted: true, data: { taskId: "srv-task-1", taskRunId: "srv-run-1", conversationId: "srv-conv-1", currentVersion: 0, state: "CREATED" } };
      }
      return { accepted: true, data: { taskId: "srv-task-1", taskRunId: "srv-run-1", conversationId: "srv-conv-1", currentVersion: 1, state: "RUNNING" } };
    }
  };

  const created = await createRemoteTask({ commandClient, taskText: "找出潜在客户", projectId: "room-1", localTaskId: "local-task-1", chiefDecisionId: "decision-1" });
  ...
  assert.equal(calls[0].input.payload.chiefDecisionId, "decision-1");
  ...
  assert.equal(calls[1].input.payload.autoConfirmLowRisk, true);
});
...
test("remote task can be primed behind the requirement and access gates", async () => {
  ...
  assert.equal(request.input.payload.autoConfirmLowRisk, true);
...
test("remote identity rejects incomplete server responses", async () => {
  assert.deepEqual(remoteTaskIdentity({ data: { task_id: "task-1", run_id: "run-1", conversation_id: "conv-1" } }), {
    taskId: "task-1", taskRunId: "run-1", conversationId: "conv-1", currentVersion: null, currentSeq: null, state: null, requirement: null
  });
