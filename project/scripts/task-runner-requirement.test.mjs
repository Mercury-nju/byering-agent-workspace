...
test("public assignment cards do not tell the user to authorize an account", async () => {
  ...
  assert.match(source, /方案已经完成，不会读取业务数据，也不会调用外部执行器/);
  assert.doesNotMatch(source, /分工只建立计划，不会触发账号登录、数据读取或对外动作。下一步由你完成授权/);
});

test("plan-only result cards render structured plan details instead of a waiting-for-data placeholder", async () => {
  const source = await readFile(new URL("../src/salebuddy/ui/task-runner.js", import.meta.url), "utf8");
  const summary = source.slice(source.indexOf("function showSummaryBox"), source.indexOf("function refreshResultCard"));
  assert.match(summary, /resultSnapshot\.plan/);
  assert.match(summary, /验收标准/);
  assert.match(summary, /hasStructuredDetails/);
  assert.match(summary, /!hasStructuredDetails/);
  assert.match(summary, /showActivity: false/);
});
