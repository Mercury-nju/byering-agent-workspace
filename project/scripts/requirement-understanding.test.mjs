...
test("proposal normalization carries the chief decision contract", () => {
  const proposal = normalizeRequirementProposal({
    title: "公开找人",
    objective: "寻找 AI 科普博主",
    scope: "抖音公开信息",
    deliverable: "候选账号清单",
    guardrail: "不读取私域数据，不执行触达",
    intent: "task",
    riskLevel: "low",
    confirmationPolicy: "none",
    responseMode: "task_card",
    requiredCapabilities: ["douyin_account_discovery"],
    blockingMissing: [],
    optionalMissing: ["候选数量"],
    defaultsApplied: ["候选数量默认 20 个"],
    userMessage: "我会直接安排找人专家执行。"
  });

  assert.equal(proposal.decision.intent, "task");
  assert.equal(proposal.decision.riskLevel, "low");
  assert.equal(proposal.decision.confirmationPolicy, "none");
  assert.equal(proposal.decision.shouldCreateTask, true);
  assert.deepEqual(proposal.decision.requiredCapabilities, ["douyin_account_discovery"]);
  assert.deepEqual(proposal.decision.optionalMissing, ["候选数量"]);
  assert.deepEqual(proposal.decision.defaultsApplied, ["候选数量默认 20 个"]);
});

test("deterministic policy prevents a model from downgrading high risk", () => {
  const proposal = normalizeRequirementProposal({
    title: "继续触达",
    objective: "继续联系拒绝用户并承诺退款",
    scope: "已拒绝用户",
    deliverable: "发送结果",
    guardrail: "模型声称可以发送",
    intent: "task",
  ...
  }, { originalInput: "用户已经明确拒绝，继续发送并承诺全额退款" });

  assert.equal(proposal.decision.riskLevel, "high");
  assert.equal(proposal.decision.confirmationPolicy, "blocking");
  assert.equal(proposal.decision.shouldCreateTask, false);
});
