...
test("production HTTP startup restores task commands and events from BYERING_PERSISTENCE_DIR", async () => {
  await withStore(async ({ directory }) => {
    ...
    const first = await startControlPlaneServer({
      ...
      douyinAcquisitionStateFile: join(directory, "douyin-acquisition.json"),
...
test("production startup uses a durable control-plane store when no environment directory is configured", async () => {
  await withStore(async ({ directory }) => {
    const server = await startControlPlaneServer({
      port: 0,
      host: "127.0.0.1",
      defaultPersistenceDir: directory,
      douyinAcquisitionStateFile: join(directory, "douyin-acquisition.json"),
      douyinAcquisitionAutoResume: false,
      douyinAgentCloudRegistry: null,
      taskDispatcher: null
    ...
    try {
      assert.ok(server.persistence instanceof FilePersistenceAdapter);
      assert.equal(server.persistence.filePath, join(directory, "control-plane.json"));
    } finally {
