...
test("find-only workflow dispatches the public prospect executor without a cloud-account uid", async () => {
  ...
  const dispatchedIndex = events.findIndex((event) => event.type === "task.execution.dispatched");
  const completedIndex = events.findIndex((event) => event.type === "prospect.discovery.completed");
  assert.notEqual(dispatchedIndex, -1);
  assert.notEqual(completedIndex, -1);
  assert.equal(dispatchedIndex < completedIndex, true);
});
...
test("control plane completes the find-only Agent chain only after risk review", async () => {
  ...
  const events = plane.listTaskEvents(created.taskId);
  const dispatchedIndex = events.findIndex((event) => event.type === "task.execution.dispatched");
  const completedIndex = events.findIndex((event) => event.type === "task.completed");
  assert.notEqual(dispatchedIndex, -1);
  assert.notEqual(completedIndex, -1);
  assert.equal(dispatchedIndex < completedIndex, true);
});
