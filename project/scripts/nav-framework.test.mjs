...
test("primary capability destination is named Agent Center", () => {
  const document = installDom();
  buildSidebarFixture(document);
  const instance = mountNavFramework({ openers: noOpOpeners() });
  const row = modeRow(document, "agentSquare");
  const iconMarkup = row?.querySelector(".sb-nav-icon")?.innerHTML || "";
  assert.equal(row?.textContent, "Agent 中心");
  assert.match(iconMarkup, /data-sb-agent-avatar="1"/);
  assert.equal((iconMarkup.match(/data-sb-agent-eye="1"/g) || []).length, 2);
  assert.equal((iconMarkup.match(/<rect/g) || []).length, 0);
  instance.unmount();
});
...
test("mounted navigation restores the office entry as the first work row", () => {
  const { document, instance } = mountFixture();
  assert.equal([...document.querySelectorAll("[data-sb-mode]")].some((row) => row.textContent === "新办公室"), false);
  assert.equal(document.querySelector('[data-sb-mode="office"]')?.textContent, "办公室");
  assert.notEqual(document.querySelector('[data-sb-mode="office"]')?.style.display, "none");
  assert.equal(document.querySelector('[data-sb-mode="office"]')?.getAttribute("aria-hidden"), null);
  assert.equal(document.querySelector('[data-sb-nav-slot="office"]')?.style.order, "11");
  instance.unmount();
});
...
test("mounted navigation exposes Agent Square as a direct route opener", () => {
  const document = installDom();
  buildSidebarFixture(document);
  let opened = 0;
  const instance = mountNavFramework({
...
test("Agent Square route forwards resumable flow options", () => {
  const document = installDom();
  buildSidebarFixture(document);
  let received = null;
  const instance = mountNavFramework({
    openers: {
      ...noOpOpeners(),
      agentSquare(options) { received = options; }
    }
  });
  const resumeFlow = { agentId: "mkt-dm-inbox", replyMode: "draft" };
  instance.openAgentSquare({ initialAgentId: "mkt-dm-inbox", resumeFlow });
  assert.equal(received.initialAgentId, "mkt-dm-inbox");
  assert.deepEqual(received.resumeFlow, resumeFlow);
  instance.unmount();
});
...
test("conversation strategy remains a deep route without becoming a primary navigation row", () => {
  ...
  const instance = mountNavFramework({
    openers: {
      ...noOpOpeners(),
      conversationStrategy(options) { strategyOptions = options; }
    }
  });
  FakeMutationObserver.flush();
...
test("programmatic conversation strategy navigation uses the same integrated route", () => {
  const document = installDom();
  buildSidebarFixture(document);
  let strategyOptions = null;
  const instance = mountNavFramework({
...
test("blank clicks in the host scroll area keep the current product page open", () => {
  ...
  const page = openPage({ title: "Agent Center" });
  const blankArea = node(document, "div", { className: "native-content-blank-area" });
  fixture.scroll.appendChild(blankArea);

  try {
    blankArea.dispatchEvent(new FakeEvent("mousedown", { bubbles: true, cancelable: true }));
    assert.equal(getCurrentPage(), page);
    assert.equal(page.root.isConnected, true);
  } finally {
    closeCurrentPage();
  }
});
...
class FakeElement extends FakeEventTarget {
  constructor(tagName, ownerDocument) {
    ...
    Object.defineProperties(this.style, {
      setProperty: { value: (name, value) => { this.style[name] = String(value); } },
      getPropertyValue: { value: (name) => this.style[name] || "" },
      removeProperty: { value: (name) => { delete this.style[name]; } }
    });
    this._text = "";
    this._innerHTML = "";
...
function modeRow(document, mode) {
  return document.querySelector(`[data-sb-mode="${mode}"], [data-sb-extra-mode="${mode}"]`);
}
...
function assertSingleActive(document, expectedMode) {
  ...
  assert.equal(classes[0].dataset.sbMode || classes[0].dataset.sbExtraMode, expectedMode);
...
test("office remains visible while mounted and restores cleanly on unmount", () => {
  const { fixture, instance } = mountFixture();

  assert.equal(fixture.office.dataset.sbMode, "office");
  assert.notEqual(fixture.office.style.display, "none");
...
test("primary navigation follows office, dialogue, Agent Center, realtime work, results child, then knowledge", () => {
  ...
  assert.equal(office.parentElement.style.order, "11");
...
test("project groups never render or poll from the primary sidebar", async () => {
  const document = installDom();
  buildSidebarFixture(document);
  const rooms = [
  ...
  const gateway = {
    action(action) {
      calls.push(action);
      if (action === "room.action.list") return Promise.resolve({ data: { rooms } });
      if (action === "room.office.current") return Promise.resolve({ data: { roomId: "room-leads" } });
  ...
  await settleDom();
  assert.equal(document.querySelector('[data-sb-nav-projects="1"]'), null);
  assert.equal(document.querySelector("[data-sb-project-id]"), null);
  assert.equal(calls.includes("room.action.list"), false);
  assert.equal(calls.includes("room.office.current"), false);
  instance.unmount();
});
...
test("native office active styling is suspended while realtime work is open and restored on return", () => {
  const { document, fixture, instance } = mountFixture();
  fixture.office.classList.add("_selfActive_xyz_");
  FakeMutationObserver.flush();
  assertSingleActive(document, "office");

  modeRow(document, "realtimeWork").click();
  const realtimeActive = activeSnapshot(document);
  assert.equal(realtimeActive.classes.length, 1);
  assert.equal(realtimeActive.aria.length, 1);
  assert.equal(realtimeActive.classes[0].dataset.sbExtraMode, "realtimeWork");
  assert.equal(realtimeActive.aria[0], realtimeActive.classes[0]);
  assert.equal(fixture.office.classList.contains("_selfActive_xyz_"), false);

  fixture.office.click();
  assertSingleActive(document, "office");
  assert.equal(fixture.office.classList.contains("_selfActive_xyz_"), true);
  instance.unmount();
});
...
test("account DOM supports login and logout actions", () => {
  ...
  assert.ok(document.querySelector("[data-sb-group=configuration]"));
...
function appendOfficeTitle(document, text = "Byering办公室") {
...
test("office switch does not inject a branded title", async () => {
  const document = installDom();
  buildSidebarFixture(document);
  const titleFixture = appendOfficeTitle(document);
  ...
  assert.equal(titleFixture.title.textContent, "Byering办公室");
});

test("office title is hidden without affecting other pages", async () => {
  const document = installDom();
  buildSidebarFixture(document);
  ...
  assert.equal(officeTitle.title.querySelector("[data-sb-office-title]"), null);
  assert.equal(officeTitle.title.querySelector("[data-sb-office-room]"), null);
  assert.match(stylesheet.textContent, /\.office-dashboard \[class\*="_pageTitleText_"\]\{display:none !important\}/);
  assert.doesNotMatch(stylesheet.textContent, /(?:^|\n)\[class\*="_pageTitleText_"\]\{display:none !important\}/);
