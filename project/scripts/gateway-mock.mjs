...
function createGatewayState({ seedOfficeDemo = false } = {}) {
  const rooms = createRoomsStore(path.join(projectRoot, "rooms"));
  const activeRoomId = seedOfficeDemo ? rooms.getActiveRoomId() : null;
  ...
  const messages = seedOfficeDemo
    ...
    : {};
  // Demo fixtures are opt-in. A normal local session must not pretend that
  // any room or subagent is already running.
  for (const room of rooms.listRooms()) {
    const active = seedOfficeDemo && room.id === activeRoomId;
...
async function actionResult(state, action, payload) {
  ...
  if (action === "chief.message.decide") {
    ...
    if (!text) return { code: -1, message: "幕僚长消息不能为空", errorCode: "CHIEF_MESSAGE_REQUIRED" };
    return ok(mockChiefDecision(text));
  }
  // ── SaleBuddy 项目组（任务房间）──
...
export function startGatewayMock({ port = 5151, seedOfficeDemo = process.env.MARVIS_SEED_OFFICE_DEMO === "1" } = {}) {
  const state = createGatewayState({ seedOfficeDemo });
