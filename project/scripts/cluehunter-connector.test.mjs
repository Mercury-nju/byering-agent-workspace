...
test("legacy ACK success/failure becomes sent or failed without allowing an unknown action to succeed", () => {
  const success = mapLegacyAckToEvents({
    ackId: "ack-4",
    actionType: 4,
    result: "SUCCESS",
    resultCode: 0,
    commandId: "letter-4",
    reqId: "task-run-4",
    consumerSecUid: "sec-uid-4",
    consumerOpenId: "sec-id-4"
  }, context);
  assert.equal(success.length, 1);
  assert.equal(success[0].type, "outreach.sent");
  assert.equal(success[0].payload.deliveryState, "success");
  assert.equal(success[0].payload.commandId, "letter-4");
  assert.equal(success[0].payload.reqId, "task-run-4");
  assert.equal(success[0].payload.secUid, "sec-uid-4");
  assert.equal(success[0].payload.secId, "sec-id-4");
...
test("legacy submit preserves video comment reply locators and routes action 23", async () => {
  const recorder = fetchRecorder([response({ code: 0, data: {
    accepted: true,
    commandId: "reply-23",
  ...
  await connector.submit({
    ...
    tenantId: "42",
    uid: "1001",
    idempotencyKey: "reply-23-once",
    actionType: 23,
    commentId: "comment-1",
    videoId: "video-1",
    videoUrl: "https://www.douyin.com/video/1",
    consumerSecUid: "target-sec-1",
    consumerOpenId: "target-open-1",
    consumerNickname: "潜客",
    operatedAccountSecId: "sender-sec-1",
    content: "你好，看到你的留言了。"
  });

  const body = JSON.parse(recorder.calls[0].init.body);
  assert.equal(body.action, 23);
  assert.equal(body.queue, "VIDEO_COMMENT_REPLY");
  assert.equal(body.commentId, "comment-1");
  assert.equal(body.videoId, "video-1");
  assert.equal(body.videoUrl, "https://www.douyin.com/video/1");
  assert.equal(body.consumerSecUid, "target-sec-1");
  assert.equal(body.consumerOpenId, "target-open-1");
  assert.equal(body.consumerNickname, "潜客");
  assert.equal(body.operatedAccountSecId, "sender-sec-1");
  assert.equal(body.content, "你好，看到你的留言了。");
});
