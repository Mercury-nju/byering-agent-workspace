...
# Optional per-Agent override used by 评论区获客管家's public comment reply
# action. These are RPA identities, not Douyin account identifiers.
BYERING_CLUEHUNTER_TENANT_ID_MKT_COMMENT_ACQUISITION=<numeric-tenant-id>
BYERING_CLUEHUNTER_UID_MKT_COMMENT_ACQUISITION=<numeric-executor-uid>
```
...
## Public comment reply boundary

评论区获客管家 uses the Douyin notification/MCP path only for reading new
comments and the ClueHunter RPA path for the mutating public reply. The reply
adapter submits `actionType=23` (`video_comment_reply`) and requires a stable
`commentId` plus `videoId` or `videoUrl`. It never downgrades a public reply to
a private message. The RPA tenant and robot uid are resolved server-side,
prefer the per-Agent variables above, and are rejected when missing or
non-numeric.
