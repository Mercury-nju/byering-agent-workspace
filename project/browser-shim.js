...
import { installPixiLegacyRoleLabelFilter } from "./src/salebuddy/office-role-skin.js?v=20260907-native-agents-1";

// This shim is guaranteed to execute before the recovered native bundle.
// Remove its retired office name sprites before Pixi builds the scene.
installPixiLegacyRoleLabelFilter({
  workbench,
  debug: new URLSearchParams(globalThis.location?.search || "").has("office_texture_debug")
});

// The recovered office runtime is mounted inside the native React bundle and
// does not publish its Pixi application. Capture that application at the
// public engine boundary so the product layer can mirror real agent state and
// drive presentation-only idle activities without replacing the renderer.
function installOfficeApplicationBridge() {
  const Application = workbench?.A;
  const init = Application?.prototype?.init;
  if (typeof init !== "function" || init.__byeringOfficeBridge) return;
  const bridgedInit = async function bridgedOfficeApplicationInit(...args) {
    const result = await init.apply(this, args);
    const applications = globalThis.__workbenchApplications || (globalThis.__workbenchApplications = []);
    if (!applications.includes(this)) applications.push(this);
    globalThis.__workbenchApplication = this;
    const current = globalThis.__gameOffice || {};
    globalThis.__gameOffice = { ...current, app: this };
    return result;
  };
  bridgedInit.__byeringOfficeBridge = true;
  Application.prototype.init = bridgedInit;
}
