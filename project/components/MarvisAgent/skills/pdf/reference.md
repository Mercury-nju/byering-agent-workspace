...
### 概述
pypdfium2 是 PDFium（Chromium 的 PDF 库）的 Python 绑定。它非常适合快速 PDF 渲染、图像生成，可作为 PyMuPDF 的替代方案。
...
> 提示：若表格内容或标题包含中文，**必须**先注册 CJK CID 字体，并在 `TableStyle` 的 `FONTNAME` 及段落样式的 `fontName` 中统一切换到该字体，否则中文会渲染为空白方块。详见 SKILL.md 中的"处理中文（CJK）字体"小节。
...
# 添加带高级样式的表格
# 注意：FONTNAME 必须使用已注册的 CJK 字体，否则中文表头/单元格会乱码
table = Table(data)
table.setStyle(TableStyle([
    ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
    ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
    ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
    ('FONTNAME', (0, 0), (-1, -1), CJK_FONT),  # 全表统一中文字体
    ('FONTSIZE', (0, 0), (-1, 0), 14),
