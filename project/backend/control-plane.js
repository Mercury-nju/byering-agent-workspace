...
function chiefConsultationGuidance(input) {
  ...
  if (/(?:金牌客服|快速接待客服).{0,20}(?:私信|咨询|接待)?|(?:私信|咨询).{0,20}(?:金牌客服|快速接待)/u.test(text)) {
    return "想用更简单的承接流程请使用金牌客服。先选择并授权账号，再配置业务资料、回复规则和人工交接边界。";
  }
  ...
  if (/(?:潜客触达专员|潜客激活专员|首次触达|待触达)|(?:联系|触达|发(?:送)?私信).{0,20}(?:潜客|客户|用户|名单|这些)?/u.test(text)) {
    return "联系已确认潜客请使用潜客触达专员。它按账号选择潜客或全部找到的人；发送前需要确认发送账号、对象和内容。";
  }
