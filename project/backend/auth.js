...
export function createControlPlaneAuth(options = {}) {
  ...
  const authRequired = production || (source.authRequired != null
  ...
  const configError = source.configError
    || (production && !entries.length
      ? new ControlPlaneAuthError("生产环境必须配置 tenant-scoped API key", {
        code: "CONTROL_PLANE_PRODUCTION_AUTH_NOT_CONFIGURED",
        statusCode: 500
      })
...
function isProductionEnvironment(source = {}) {
  if (source.production === true) return true;
  const configured = String(source.environment || source.deploymentMode || "").trim().toLowerCase();
  if (["production", "prod"].includes(configured)) return true;
  const environment = String(process.env.NODE_ENV || process.env.BYERING_DEPLOYMENT_MODE || "").trim().toLowerCase();
  return ["production", "prod"].includes(environment);
}
