...
// Douyin's direct login route requires platform-issued OAuth parameters and
// redirects to an empty-ticket callback when opened without them. Start from
// the normal site so the user can complete login in the real browser workspace.
const DEFAULT_LOGIN_URL = "https://www.douyin.com/";
...
export function createBrowserWorkspaceService({
  ...
  loginUrl = normalizeLoginUrl(process.env.BYERING_DOUYIN_LOGIN_URL || DEFAULT_LOGIN_URL)
} = {}) {
  ...
  async function startWorkspace({ tenantId, provider = "douyin", accountKey, accountLabel = null, accountIdentity = null, uniqueId = null, profileUrl = null, uid = null, secId = null, executorUid = null, taskId = null, scopes = [], cloudDesktop = null } = {}) {
    ...
    if (existing && existing.state !== WORKSPACE_STATES.DESTROYED) {
      ...
      const identity = normalizeAccountIdentity(accountIdentity || { uniqueId, profileUrl, uid, secId, accountName: accountLabel });
      if (identity) existing.accountIdentity = identity;
      ...
      if ((existing.state !== WORKSPACE_STATES.READY || /^about:blank$/i.test(String(existing.pageUrl || ""))) && typeof existing.handle?.navigate === "function") {
    ...
    const record = {
      ...
      accountKey,
      accountLabel,
      accountIdentity: normalizeAccountIdentity(accountIdentity || { uniqueId, profileUrl, uid, secId, accountName: accountLabel }),
  ...
  async function authorize(sessionId) {
    ...
    if (record.authenticationVerified !== true || !hasExternalAccountIdentity(record.accountIdentity)) {
      throw new BrowserWorkspaceError("已检测到抖音登录，但暂未识别账号身份", {
        code: "ACCOUNT_IDENTITY_PENDING",
        statusCode: 409,
        details: { sessionId, state: record.state, pageUrl: record.pageUrl || record.authUrl }
      });
    }
  ...
  async function execute(sessionId, input = {}) {
    ...
    if (record.authenticationVerified !== true || !hasExternalAccountIdentity(record.accountIdentity)) {
      throw new BrowserWorkspaceError("抖音账号身份尚未确认，不能执行真实动作", {
        code: "ACCOUNT_IDENTITY_PENDING",
        statusCode: 409,
        details: { sessionId }
      });
  ...
  async function refresh(record) {
    ...
    if (typeof status.authenticationVerified === "boolean") {
      record.authenticationVerified = status.authenticationVerified;
    } else if (status.ready !== true) {
      record.authenticationVerified = false;
    }
    const observedIdentity = normalizeAccountIdentity(status.accountIdentity || status.identity);
    if (observedIdentity) record.accountIdentity = { ...record.accountIdentity, ...observedIdentity };
    if (record.accountIdentity && !hasExternalAccountIdentity(record.accountIdentity)) {
      // A managedAccountKey identifies our workspace slot, not the external
      // Douyin user. Never retain it as if it were a platform identity.
      record.accountIdentity = null;
    }
    record.executorUid = status.executorUid || status.robotUid || status.uid || record.executorUid || null;
    if (status.ready === true) {
      record.state = WORKSPACE_STATES.READY;
    }
  ...
  async function ensureHandle(record) {
    ...
    record.authUrl = normalizeLoginUrl(record.authUrl, loginUrl);
    ...
    try {
      record.handle = await launcher.launchPersistent({
        ...
        headless: process.env.BYERING_BROWSER_HEADLESS === "1",
        // A persistent context can start on about:blank after a restart even
        // when the registry says READY. Always restore an empty page to the
        // real Douyin site; the persistent profile keeps its login state.
        openLogin: record.state !== WORKSPACE_STATES.READY,
        restore: true
      });
      // A restored persistent context may come back with an about:blank page
      // after the previous process exited. Put unverified sessions back on
      // the real Douyin site so the user can continue authorization immediately.
      if (record.state === WORKSPACE_STATES.READY
        && typeof record.handle.navigate === "function") {
        await record.handle.navigate(record.authUrl || loginUrl).catch(() => {});
        record.pageUrl = record.authUrl || loginUrl;
      }
    } catch (cause) {
  ...
  function restoreRegistry() {
    ...
    for (const value of parsed.sessions) {
      ...
      if (!record) continue;
      record.authUrl = normalizeLoginUrl(record.authUrl, loginUrl);
      if (record.state !== WORKSPACE_STATES.READY && isBrokenLoginCallback(record.pageUrl)) {
        record.pageUrl = record.authUrl;
      }
  ...
  function redactSnapshot(record) {
    return {
      ...
      authenticationVerified: record.authenticationVerified === true,
  ...
  return {
    async launchPersistent(options) {
      ...
      if (options.openLogin !== false) {
        await navigateToLoginPage(page, options.loginUrl);
      }
      return {
        async navigate(url) {
...
async function navigateToLoginPage(page, loginUrl) {
  let lastError = null;
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      await page.goto(loginUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
      if (!/^about:blank$/i.test(page.url())) return true;
    } catch (error) {
      lastError = error;
    }
  }
  // Keep the browser useful even when the first navigation races Chrome
  // startup. The fallback is the same safe site, without OAuth parameters.
  if (!/^about:blank$/i.test(page.url())) return true;
  try {
    await page.goto(DEFAULT_LOGIN_URL, { waitUntil: "commit", timeout: 15000 });
    return !/^about:blank$/i.test(page.url());
  } catch (error) {
    lastError = error;
  }
  if (lastError) console.warn(`[browser-workspace] Douyin login navigation failed: ${lastError.message}`);
  return false;
}
...
async function inspectRealDouyinSession(context, page) {
  ...
  const accountIdentity = await extractDouyinIdentity(activePage);
  return {
    ready: isDouyinSessionReady({ cookieNames, url }),
    authenticationVerified: isDouyinSessionReady({ cookieNames, url }),
    url,
    accountLabel: accountIdentity?.accountName || null,
    ...(accountIdentity ? { accountIdentity } : {})
  };
}

async function extractDouyinIdentity(page) {
  if (!page || typeof page.evaluate !== "function") return null;
  try {
    return await page.evaluate(() => {
      // Only inspect the signed-in account affordance. Feed cards also link
      // to /user pages, so scanning every anchor would bind the wrong person.
      const link = document.querySelector('header a[href*="/user/"], nav a[href*="/user/"], [data-e2e*="avatar"] a[href*="/user/"]');
      const profileUrl = link?.href && /douyin\.com\/user\//i.test(link.href)
        && !/\/user\/(?:self|undefined)(?:[/?#]|$)/i.test(link.href)
        ? link.href
        : null;
      if (!profileUrl) return null;
      const accountName = String(link.textContent || "").replace(/\s+/g, " ").trim();
      const secId = profileUrl.match(/\/user\/([^/?#]+)/i)?.[1] || null;
      return {
        ...(accountName ? { accountName } : {}),
        profileUrl,
        ...(secId ? { secId: decodeURIComponent(secId) } : {})
      };
    });
  } catch {
    return null;
...
function normalizeAccountIdentity(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const profileUrl = firstText(value.profileUrl, value.profile_url, value.homepageUrl, value.homepage_url);
  const uniqueId = firstText(value.uniqueId, value.unique_id, value.douyinId, value.douyin_id);
  const uid = firstText(value.uid, value.userId, value.user_id, value.platformUid, value.platform_uid);
  const secId = firstText(value.secId, value.sec_id, value.secUid, value.sec_uid);
  const accountName = firstText(value.accountName, value.account_name, value.nickname, value.name);
  const managedAccountKey = firstText(value.managedAccountKey, value.managed_account_key, value.accountKey, value.account_key);
  if (!profileUrl && !uniqueId && !uid && !secId && !managedAccountKey) return null;
  return {
    ...(accountName ? { accountName } : {}),
...
function hasExternalAccountIdentity(identity) {
  if (!identity || typeof identity !== "object" || Array.isArray(identity)) return false;
  return [identity.profileUrl, identity.uniqueId, identity.uid, identity.secId]
...
function normalizeLoginUrl(value, fallback = DEFAULT_LOGIN_URL) {
  try {
    const url = new URL(String(value || fallback));
    if (isBrokenLoginCallback(url.href)
...
function isBrokenLoginCallback(value) {
  try {
    const url = new URL(String(value || ""));
    return /\/passport\/sso\/login\/callback\/?$/i.test(url.pathname)
      && !url.searchParams.get("ticket");
  } catch {
    return /\/passport\/sso\/login\/callback\/?(?:\?|$)/i.test(String(value || ""));
  }
}
