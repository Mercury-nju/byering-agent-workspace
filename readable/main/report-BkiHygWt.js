...
if (!E)
  throw new Error(
    "[marvis-card] window.__MARVIS_REACT_JSX_RUNTIME__ 未注入；请确认主端已调用 installMarvisCardHostRuntime() 暴露 React 实例",
  );
const { Fragment: N, jsx: S, jsxs: D } = E,
  g = window.__MARVIS_REACT__;
if (!g)
  throw new Error(
    "[marvis-card] window.__MARVIS_REACT__ 未注入；请确认主端已调用 installMarvisCardHostRuntime() 暴露 React 实例",
  );
const {
    Activity: P,
    Children: k,
...
function Se(e) {
  ...
  R(() => {
    const o = {};
    (r && (o.message_id = r),
      t?.source && (o.conversation_platform = t.source),
      n !== void 0 && (o.order_brand = String(n)),
      s && (o.conversation_id = s),
      C(o));
  }, [r, t, n, s]);
}
function pe(e, t = {}, r) {
...
const A = "marvis_client",
  O = "fpzkeTlcGLVP37qiOWd29MyaOqaJYxMi";
let f = null;
function T(e) {
  f = e;
}
async function he(e, t) {
  if (!f)
    throw new Error(
      "[ualAccess] 宿主请求能力未注入：请确认卡片入口已调用 useHostRequest(api)",
    );
  return f({
    path: e,
    body: t,
...
function be(e, t = {}) {
  ...
  for (const [s, o] of Object.entries(r))
    o != null &&
      n.push(`${encodeURIComponent(s)}=${encodeURIComponent(String(o))}`);
  return n.join("&");
}
export {
  j as F,
